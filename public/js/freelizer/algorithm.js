...
export const autoCorrelate = (buf, sampleRate) => {
  ...
  const rms1 = Math.sqrt(sum / buf.length);

  // Convert the RMS to decibels (dB)
  const dB = 20 * Math.log10(rms1);  

  if (RMS < 0.001) return NaN

  const arraySum = buf.reduce((a, value) => a + value, 0);
  const volume = Math.round(arraySum / buf.length);
  ...
  return [sampleRate / (a ? T0 - b / (2 * a) : T0),dB , volume]
}
